...
{
    public partial class AffLinksTiki
    {
        ...
        private async Task GetAffiliateUrlAsync(string url)
        {
            if (CurrentUser.Id == null || url.HasNoValue()) return;
            NewUserAffiliate.AffiliateProviderType = AffiliateProviderType.Tiki;

            if (url.ToLower().Contains(MarketplaceType.Tiki.ToString().ToLower()))
            {
                NewUserAffiliate.MarketplaceType = MarketplaceType.Tiki;
            }
            else
            {
                await Message.Error(Lv[ApiDomainErrorCodes.UserAffiliates.InvalidTikiUrl]);
                return;
            }
            ...
            if (affiliate == null)
            {
                await Message.Error(Lv[ApiDomainErrorCodes.UserAffiliates.ShortlinkCreationFailure]);
                return;
            }
            else
            {
