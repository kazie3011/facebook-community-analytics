                ...
                .ConfigureAppConfiguration((hostingContext, config) =>
                {
                    ...
                    if (!env.IsProduction())
                    {
                        config
                            ...
                            .AddJsonFile(Path.Combine(env.ContentRootPath, "..", "Configs/payrollconfigs.json"), true, true)
                            .AddJsonFile(Path.Combine(env.ContentRootPath, "..", $"Configs/payrollconfigs.{env.EnvironmentName}.json"), true, true);
                    }
                    else
                    {
                        config
                            ...
                            .AddJsonFile("Configs/payrollconfigs.json", true, true)
                            .AddJsonFile($"Configs/payrollconfigs.{env.EnvironmentName}.json", true, true);
                    }
                })
                .ConfigureServices((hostContext, services) =>
                {
                    services.AddApplication<BackgroundJobModule>();
                });
    }
}
