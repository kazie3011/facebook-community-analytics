    ...
    public class Program
    {
        public static int Main(string[] args)
        {
            ...
            Log.Logger = new LoggerConfiguration()
                ...
                //         IndexFormat = "gdll-log-identity-{0:yyyy.MM}"
                //     })
            ...
            try
            {
                Log.Information("Starting FacebookCommunityAnalytics.Api.IdentityServer.");
                CreateHostBuilder(args).Build().Run();
                return 0;
            }
            catch (Exception ex)
            {
                Log.Fatal(ex, "FacebookCommunityAnalytics.Api.IdentityServer terminated unexpectedly!");
                return 1;
            }
            finally
            {
